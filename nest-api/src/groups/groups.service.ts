...
async findGroupsByUser(userId: string): Promise<any[]> {
 ...
 async findAllByStatus(status: string): Promise<any[]> {
  ...
  async leaveGroup(groupId: string, userId: string) {
    ...
    const group = await this.findOne(groupId);
if (group.status === GroupStatus.FAILED || group.status === 'LOCKED') {
throw new BadRequestException(`Cannot leave a group that is ${group.status}`);
    }
if (group.deadline && new Date() > new Date(group.deadline)) {
    throw new BadRequestException('Cannot leave after the deadline has passed');
  }
  ...
  async decrementJoinedCount(id: string): Promise<void> {
    ...
    if (group) {
    const now = new Date();
    const isBeforeDeadline = !group.deadline || new Date(group.deadline) > now;

    // הלוגיקה: רק אם הסטטוס הוא COMPLETED והתפנה מקום (ולא LOCKED)
    if (group.status === GroupStatus.COMPLETED && 
        group.joined_count < group.target_members && 
        isBeforeDeadline) {
      
      await this.groupRepo.update(id, { status: GroupStatus.OPEN });
      this.logger.log(`Group ${id} returned to OPEN status.`);
    }
  }
}
async getMyActiveGroups(userId: string) {
  const now = new Date();
  const hideThreshold = new Date(now.getTime() - 24 * 60 * 60 * 1000);

  // שינוי מ-this.groupRepository ל-this.groupRepo (כפי שמוגדר ב-constructor שלך)
  return this.groupRepo.createQueryBuilder('group') 
    .leftJoin('group.memberships', 'membership')
    .where('membership.userId = :userId', { userId })
    .andWhere(new Brackets(qb => {
      qb.where('group.deadline > :now', { now }) 
        .orWhere(
          'group.deadline <= :now AND group.joined_count < group.target_members AND group.deadline > :hideThreshold',
          { now, hideThreshold }
        ); 
    }))
    .getMany();
