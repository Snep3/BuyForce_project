...
async function seed() {
    ...
    // ניקוי מסודר (FK-safe) + מאפס SERIAL של comments
  ...
  `);
    ...
    await groupMemberRepo.save([
        ...
        groupMemberRepo.create({ groupId: g2.id, userId: user2.id, quantity: 1 }),

        groupMemberRepo.create({ groupId: g3.id, userId: admin.id, quantity: 1 }),
    ]);

    // === 5) Comments (3) ===
    // שים לב: Comment entity דורש user + productId
    await commentRepo.save([
        commentRepo.create({
            content: 'Looks great! When is the deadline?',
            user: user1,
            productId: p1.id,
        }),
        commentRepo.create({
            content: 'Is there warranty included?',
            user: user2,
            productId: p2.id,
        }),
        commentRepo.create({
            content: 'I joined! Waiting for more people.',
            user: admin,
            productId: p3.id,
        }),
    ]);
    ...
    // === 6) Wishlist (3) ===
    // שים לב: בעמודות שלך זה snake_case: user_id / product_id / group_id
    ...
    await notificationRepo.save([
        ...
        notificationRepo.create({
            user: user2,
            type: 'WISHLIST_ADDED',
            message: `Added ${p1.name} to wishlist`,
            isRead: false,
        }),
        notificationRepo.create({
            user: admin,
            type: 'ADMIN_ALERT',
            message: 'New activity in groups',
            isRead: true,
        }),
    ]);

    // === 8) Orders (2) + OrderItems (3) ===
    ...
    const order2 = await orderRepo.save(
        orderRepo.create({
            ...
            groupId: null,
            status: 'paid',
            totalPrice: String(Number(p2.price) + Number(p3.price)),
        }),
    );
    ...
    await orderItemRepo.save([
        ...
        orderItemRepo.create({
            ...
            productId: p2.id,
            quantity: 1,
            unitPrice: String(p2.price),
            totalPrice: String(p2.price),
        }),
        orderItemRepo.create({
            orderId: order2.id,
            productId: p3.id,
            quantity: 1,
            unitPrice: String(p3.price),
            totalPrice: String(p3.price),
        }),
    ]);

    console.log('✅ Seed completed');
    console.log('Test login: admin1@test.com / 123456');
    console.log('Test login: user1@test.com / 123456');
    console.log('Test login: user2@test.com / 123456');
